...
(),PCThreadCache.getInstance().getProperty("TCID"));
	...
	public Boolean SCRCreateAccountNote() throws Exception {
		...
		if (common.WaitUntilClickable(Common.o.getObject("eleCreateNewAccountNumber"), Integer.valueOf(HTML.properties.getProperty
...
("methodName"), "System should display Account File Summary page", "System displayed Account File Summary page", "PASS");
			status = true;
		} else {
			logger.info("Account Notes not created and System not displayed Account File Summary");
			HTML.fnInsertResult(PCThreadCache.getInstance().getProperty("testcasename"), PCThreadCache.getInstance().getProperty

("methodName"), "System should display Account File Summary page", "System not displayed Account File Summary page", "PASS");
			status = false;
		}
		return status;
	}
