...
public class Common {
	...
	public void highlightElement(By locator) throws Exception {
		...
		if (HTML.properties.getProperty("HighlightElements").equalsIgnoreCase("true")) {
			String attributevalue = "border:10px solid green;";// change border
																// width and
																// colour values
																// if required
	...
	public Boolean SafeAction(By element, String value, String ColumnName) throws Exception {
		...
		if (returnValue) {
			...
			/*
			 * case "SLJ": boolean listStatus1 = false; // WebElement element1 =
			 * DriverManager.getInstance().getWebDriver().findElement(element); ele.click();
			 * WaitForPageToBeReady(); ManagerDriver.getInstance().getWebDriver
			 * ().findElement(element).sendKeys(Keys.ARROW_DOWN); WaitForPageToBeReady();
			 * List<WebElement> gwListBox1 = ManagerDriver
			 * .getInstance().getWebDriver().findElements(By.tagName("LI")); for (int i = 0;
			 * i<gwListBox1.size(); i++) { String strListValue =
			 * gwListBox1.get(i).getText(); if
			 * (strListValue.toUpperCase().contains(value.toUpperCase())) {
			 * gwListBox1.get(i).click(); returnValue = SCRCommon.JavaScript(js); listStatus
			 * = true; //logger.info("Selected item '"+ value +"' from '" + objectName +
			 * "' listbox and element '"+ element + "'"); HTML.fnInsertResult(PCThreadCache
			 * .getInstance().getProperty("testcasename"),
			 * PCThreadCache.getInstance().getProperty("methodName"),
			 * "System should Select item '"+ value +"' from '" + objectName + "' listbox",
			 * "Selected item '"+ value +"' from '" + objectName + "' listbox", "PASS");
			 * break; } } if(!listStatus1) { returnValue = false;
			 * //logger.info("Value not available '"+ value +"' in '" + objectName +
			 * "' listbox and element '"+ element + "'"); HTML.fnInsertResult
			 * (PCThreadCache.getInstance().getProperty("testcasename"),
			 * PCThreadCache.getInstance().getProperty("methodName"),
			 * "System should Select item '"+ value +"' from '" + objectName + "' listbox",
			 * "Item '"+ value +"' not available in '" + objectName + "' listbox", "FAIL");
			 * break; } break;
			 */
			}
			// WaitForPageToBeReady();
	 ...
	 */
	/*
	 * public void WaitForPageToBeReady1() { WebDriverWait wait = new
	 * WebDriverWait(ManagerDriver.getInstance() .getWebDriver(),
	 * Integer.parseInt(HTML.properties .getProperty("VERYLONGWAIT")));
	 * Predicate<WebDriver> pageLoad = new Predicate<WebDriver>() {
	 * 
	 * @Override public boolean apply(WebDriver input) { return
	 * ((JavascriptExecutor) input).executeScript(
	 * "return document.readyState").equals("complete"); } };
	 * System.out.println("Page is loaded"); //wait.until(pageLoad); }
	 */
	...
	public Boolean OpenApp() throws Exception {
		 ...
		 * PCThreadCache.getInstance().getProperty("methodName"),
		 * "Application should Open successfully" ,"Application Opened successfully",
		 * "PASS"); bStatus = true; }
		 */
		return bStatus;
	}
	 ...
	 */
	/*
	 * public void TerminateFail() throws Exception { String execution =
	 * HTML.properties.getProperty("TypeOfAutomation"); if
	 * (execution.toUpperCase().contains("HEADLESS")) { service.stop();
	 * logger.info("phantomjs service stoped"); } else { try{ Alert al =
	 * ManagerDriver.getInstance().getWebDriver().switchTo().alert(); al.dismiss();
	 * } catch(Exception e) { logger.info("Alert not found"); }
	 * ManagerDriver.getInstance().getWebDriver().quit();
	 * logger.info("WebDriver Quit"); } ScreenVideoCapture.stopVideoCapture(HTML.
	 * properties.getProperty("testcasename")); }
	 */
	...
	public boolean RunTest1(String strRunMode, String strTestCaseName, String DataSheetName, String Region) throws Exception {
					...
					try {
						...
						if (ColName.contains("Component")) {
							...
							if (!methodName.isEmpty()) {
								 ...
								 * Class.forName("com.pc.screen." + methodName); Object obj = cls.newInstance();
								...
								Object obj = cls.newInstance();
								// Method method = cls.getDeclaredMethod("SCR" +
								// methodName, Common.class);
		...
		/*
		 * if(strYES) { //logger.info("No test case selected as 'YES' in Data sheet"); }
		 */
		// Graph report code
		if (testCaseType != null && testCaseType.length() > 0 && "Regression".equalsIgnoreCase(testCaseType) && HTML.properties.getProperty("DataBaseUpdate").equalsIgnoreCase("YES")) {
			if (isTestCasePass) {
						...
						if (ColName.contains("Component")) {
							...
							if (!methodName.isEmpty()) {
								...
								if (methodName.contains("_")) {
									...
									Object obj = cls.newInstance();
									// Method method =
									// cls.getDeclaredMethod("SCR" + methodName,
									// Common.class);
	...
	/*
	 * //Deprecated Do Not Use public Recordset GetDataFromExcelDoNotUse(String
	 * strFileName, String strQuery1) throws FilloException { Fillo fillo=new
	 * Fillo(); Connection connection=(Connection) fillo.getConnection(strFileName);
	 * Recordset recordset = connection.executeQuery(strQuery1); return recordset; }
	 * 
	 * //Deprecated Do Not Use public void UpdateDataInExcelDoNotUse(String
	 * strFileName, String strQuery) throws FilloException { Fillo fillo=new
	 * Fillo(); Connection connection=fillo.getConnection(strFileName);
	 * connection.executeUpdate(strQuery); connection.close(); }
	 * 
	 * //Deprecated Do Not Use public void UpdateStatusInExcelDoNotUse(String
	 * strFileName, String strQuery) throws FilloException { Fillo fillo=new
	 * Fillo(); Connection connection=fillo.getConnection(strFileName);
	 * connection.executeUpdate(strQuery); connection.close(); }
	 */
	...
	/**
	 * @function This function use to verify the text from the table according to
	 *           the column
	 * @param obj
	 * @param readTextCol
	 *            ,readTextRow
	...
	/**
	 * @function Ability to send text to the element
	 * @param bylocator
	 * @param iWaitTime
	 * @param sValue
	 * @return true/false
	...
	/*
	 * public Boolean waitForVisibilityOfEleByXpath(final WebDriver driver, String
	 * elementXpath) { FluentWait<By> fluentWait = new
	 * FluentWait<By>(By.xpath(elementXpath)); fluentWait.pollingEvery(100,
	 * TimeUnit.MILLISECONDS); fluentWait.withTimeout(120, TimeUnit.SECONDS);
	 * fluentWait.until(new Predicate<By>() { public boolean apply(final By by) {
	 * try {
	 * 
	 * fluentWaitStatus = driver.findElement(by).isDisplayed(); return
	 * fluentWaitStatus; } catch (Exception ex) {
	 * 
	 * return fluentWaitStatus; } } }); return fluentWaitStatus; }
	 */

	// Added from PolicyInfo - R3
	...
	public Boolean OpenApp_WEISS1() throws Exception {
		Boolean bStatus = false;
		String sURL = null;
		logger.info("Browser Opened Successfully");
		sURL = HTML.properties.getProperty("WEISSRegion");
