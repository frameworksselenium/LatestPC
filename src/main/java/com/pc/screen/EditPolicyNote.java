...
(),PCThreadCache.getInstance().getProperty("TCID"));
	...
	public Boolean SCREditPolicyNote() throws Exception {
		 ...
		 * "System should display Policy Note Search Page"
		 * ,"System displayed Policy Note Search Page", "PASS"); status = true; } else {
		 * logger.info("System not displayed Policy Note Search Page");
		 * HTML.fnInsertResult(PCThreadCache.getInstance().getProperty("testcasename"),
		 * PCThreadCache.getInstance().getProperty("methodName"),
		 * "System should display Policy Note Search Page"
		 * ,"System not displayed Policy Note Search Page", "FAIL"); status = false; }
		 */
		return status;
	}
