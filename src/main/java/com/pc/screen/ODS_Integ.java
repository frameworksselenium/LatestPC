...
class ODS_Integ {
	...
	public static String strQuerySheet = "Queries_Integ";
	static Logger logger = 

Logger.getLogger(sheetname);
...
PCThreadCache.getInstance().getProperty("methodName"), "Execute ODS Component", "ODS Component was not Successfully Executed", "FAIL");
...
Status;
	...
	public static Boolean GetDataFromDB(String strQueryRefID,String strDBFields) throws Exception {
		Boolean Status = false;
...
strRoleName = null;
		...
		String[] sDBFields=strDBFields.split(":::");
...
ArrayList<HashMap<String, Object>> resultListRows2 = new ArrayList<HashMap<String, Object>>();
		switch (HTML.properties.getProperty
...
HTML.properties.getProperty("INTODSSchema");
			URL = HTML.properties.getProperty("INTURL");

			UserName = 

HTML.properties.getProperty("INTMDMODSUserID");
...
HTML.properties.getProperty("INTURL");

			UserName = HTML.properties.getProperty("INTMDMODSUserID");
			Password 

= HTML.properties.getProperty("INTMDMODSPassword");
			break;
		case "LTI":
			Schema = 

HTML.properties.getProperty("LTIODSSchema");
			URL = HTML.properties.getProperty("INTURL");
...
HTML.properties.getProperty("QA02ODSSchema");
			URL = HTML.properties.getProperty("QAURL");

			UserName = 

HTML.properties.getProperty("QAMDMODSUserID");
			Password = HTML.properties.getProperty("QAMDMODSPassword");
			

break;
		case "DLTQ02":
			Schema = HTML.properties.getProperty("LTQ02ODSSchema");
			URL = 
...
HTML.properties.getProperty("LTI02ODSSchema");
			URL = HTML.properties.getProperty("INTURL");
...
break;
		...
		whereConstraint.put("Query_REF_ID", strQueryRefID);
		resultListRows = sXL.executeSelectQuery

(strQuerySheet, whereConstraint);
		//FlatFile sReadWrite = FlatFile.getInstance();
		
		String sFileName = 

PCThreadCache.getInstance().getProperty(PCConstants.Integ_FlatFile);
		FlatFile sReadWrite=FlatFile.getInstanceCSV("READ", 

sFileName,"csv");
		for (HashMap<String, Object> ProcessRow : resultListRows) {
			String sQueries = (String) 

ProcessRow.get("Queries");
...
{
				strAccountNumber = sReadWrite.Read("Account No", sFileName);
				strPolicyNumber = 

sReadWrite.Read("PolicyNumber", sFileName);
...
sReadWrite.Read("Account No", sFileName);
				sQueries = sQueries.replace("$AccountNumber$", strAccountNumber);
		...
		strPolicyNumber = sReadWrite.ReadCSV("PolicyNumber",sFileName,"||");
				sQueries = sQueries.replace

("$PolicyNumber$", strPolicyNumber);
				sQueries = sQueries.replace("$SchemaName$", Schema);
			} else if 

(sQueries.contains("$FPolicyNumber$")) {
				strPolicyNumber = sReadWrite.Read("FPolicyNumber", 

scrCommon.FlatFileName());
...
strGroupName = sReadWrite.Read("GroupName", sFileName);
	...
	strQueueName = sReadWrite.Read("QueueName", sFileName);
				sQueries = sQueries.replace("$QueueName$", strQueueName);
	

			sQueries = sQueries.replace("$SchemaName$", Schema);
			} else if (sQueries.contains

("$RewritePolicyNumber$")) {
				strRewritepolicy = sReadWrite.Read("RewritePolicyNumber", sFileName);
				

sQueries = sQueries.replace("$RewritePolicyNumber$", strRewritepolicy);
				sQueries = sQueries.replace("$SchemaName$", 

Schema);
			} else if (sQueries.contains("$OrganizationName$")) {
				strOrganizationName = 

sReadWrite.Read("OrganizationName", scrCommon.FlatFileName());
				sQueries = sQueries.replace("$OrganizationName$", 

strOrganizationName);
				sQueries = sQueries.replace("$SchemaName$", Schema);
			} else if 

(sQueries.contains("$UserFirstName$") && sQueries.contains("$UserLastName$")) {
				strUserFirstName = sReadWrite.Read

("UserFirstName", sFileName);
				strUserLastName = sReadWrite.Read("UserLastName", sFileName);
				

sQueries = sQueries.replace("$UserFirstName$", strUserFirstName);
				sQueries = sQueries.replace("$UserLastName$", 

strUserLastName);
				sQueries = sQueries.replace("$SchemaName$", Schema);
			} else if 

(sQueries.contains("$ActivityId$")) {
		...
		sQueries = sQueries.replace("$ActivityId$", strActivityID);
				...
				strRoleName = sReadWrite.Read("RoleName", 

scrCommon.FlatFileName());
				sQueries = sQueries.replace("$RoleName$", strRoleName);
				sQueries 

= sQueries.replace("$SchemaName$", Schema);
			}

			int odsWait = Integer.valueOf

(HTML.properties.getProperty("ODSWAIT"));
...
(fieldValue.toUpperCase().equalsIgnoreCase(queryColumnName.toUpperCase())) 
							{
...
sConvertedFormatPC = null;
		String sConvertedFormatDB = null;
		String splitValue[] = null;
		switch 

(CompareFormat.toUpperCase()) {
		case "TEXT":
		...
		break;
		case "REMOVESPACE":
			Status = RemoveSpaceCompareResults(sPCValue, sDBValue, sPCFieldName, 
...
break;
		case "SPACECONTAINS":
			...
			Status = CompareResults

(sPCValue, sDBValue, sPCFieldName, sDBAliasName);
			break;
		case "DATETIME":
			try {
...
sConvertedFormatDB = sdfDestination.format(date);
				...
				SimpleDateFormat sdfDestination1 
...
sConvertedFormatDB = sdfDestination.format(date);
				Status = CompareResults(sPCValue, sConvertedFormatDB, 

sPCFieldName, sDBAliasName);
			} catch (ParseException exp) {
				exp.printStackTrace();
			}
		...
		case "DATE1":
			try {
				SimpleDateFormat defFormat = new 
...
CompareResults(sConvertedFormatPC, sConvertedFormatDB, sPCFieldName, sDBAliasName);
			} catch (ParseException exp) {
...
defFormat1.parse(sPCValue);
				SimpleDateFormat sdfDestination1 = new SimpleDateFormat("MM/dd/yyyy");
				

sConvertedFormatPC = sdfDestination1.format(date1);
				Status = CompareResults(sConvertedFormatPC, sConvertedFormatDB, 
...
(sConvertedFormatPC, sDBValue, sPCFieldName, sDBAliasName);
			break;
		/*
...
"Administrative Fund Assessment Surcharge":
...
splitValue = sDBValue.split("-");
			...
			System.out.println("SplitValue of " + 
...
case "PRODUCERCODE":
			...
			Status = CompareResults(sPCValue, sDBValue, 
...
CompareResults(sPCValue, sDBValue, sPCFieldName, sDBAliasName);
		}
		if (!Status) {
			Status = true;
		}
	...
	private boolean CompareResults(String sPCValue, String sDBValue, String sPCFieldName, String sDBAliasName) throws 
...
PCThreadCache.getInstance().getProperty("methodName"), "PC " + sPCFieldName + " Value('" + sPCValue + "') should match with DB " + sDBAliasName + " Value('" + sDBValue + "')", "PC " + sPCFieldName + " Value('" + sPCValue + "') is matching with DB " + sDBAliasName + " Value('" + sDBValue + 
...
().getProperty("testcasename"), PCThreadCache.getInstance().getProperty("methodName"), "PC " + sPCFieldName + " Value('" + sPCValue + "') should match with DB " + sDBAliasName + " Value('" + sDBValue + "')", "PC " + sPCFieldName + " Value('" + sPCValue + "') is not matching with DB " + 

sDBAliasName + " Value('" + sDBValue + "')", "FAIL");
		}
		return Status;
	}
...
RemoveSpaceCompareResults(String sPCValue, String sDBValue, String sPCFieldName, String sDBAliasName) throws IOException {
		Boolean 
...
(sPCValue.toUpperCase().equals(sDBValue.toUpperCase()) || sDBValue.toUpperCase().equals(sPCValue.toUpperCase())) {
			

HTML.fnInsertResult(PCThreadCache.getInstance().getProperty("testcasename"), PCThreadCache.getInstance().getProperty("methodName"), "PC " + 

sPCFieldName + " Value('" + sPCValue + "') should match with DB " + sDBAliasName + " Value('" + sDBValue + "')", "PC " + sPCFieldName + " Value('" + sPCValue + "') is matching with DB " + sDBAliasName + " Value('" + sDBValue + "')", "PASS");
			Status = true;
		} 

else {
			HTML.fnInsertResult(PCThreadCache.getInstance().getProperty("testcasename"), PCThreadCache.getInstance().getProperty("methodName"), "PC " + sPCFieldName + " Value('" + sPCValue + "') should match with DB " + sDBAliasName + " Value('" + sDBValue + "')", "PC " + 

sPCFieldName + " Value('" + sPCValue + "') is not matching with DB " + sDBAliasName + " Value('" + sDBValue + "')", "FAIL");
		}
...
return Status;
	...
	public static ResultSet ConnectDB(String strSQLStatement, String strURL, String strUserName, String strPassword) throws 

ClassNotFoundException, SQLException {
		Class.forName("oracle.jdbc.driver.OracleDriver");
		Connection ucaConn = 

DBConnectionManager.getInstance().getConnectionFromPool(); 

		//Connection conn = DriverManager.getConnection(strURL, strUserName, strPassword);
		Statement stment = ucaConn.createStatement();
...
return rs;
	...
	/*
	 * private String FlatFileName() { XlsxReader xls = XlsxReader.getInstance();
	 * String tcID = 

PCThreadCache.getInstance().getProperty("TCID"); String
	 * sFileName = null; HashMap<String,Object> whereConstraint = new
	 * 

HashMap<String,Object>(); ArrayList<HashMap<String,Object>> resultListRows=
	 * new ArrayList<HashMap<String,Object>>(); 

whereConstraint.clear();
	 * whereConstraint.put(PCConstants.ID, tcID); resultListRows =
	 * xls.executeSelectQuery

(PCConstants.SHEET_FLATFILE, whereConstraint); for
	 * (HashMap<String,Object> processRow: resultListRows) { sFileName =
	 * (String)

processRow.get("FlatFileName");
	 * 
	 * } return sFileName;
	 * 
	 * }
...
whereConstraint.put(PCConstants.ID, splittcID[0]);
		resultListRows = xls.executeSelectQuery(PCConstants.SHEET_FLATFILE, 
...
getDBValueForQueryInput(String strFuncValue) throws Exception {
		Boolean Status = false;
		Boolean sDBAliasName = false;
...
break;
		case "LTQ2":
			Schema = HTML.properties.getProperty("LTQ2ODSSchema");
			URL = 
...
HTML.properties.getProperty("LTI2ODSSchema");
			URL = HTML.properties.getProperty("INTURL");
...
sReadWrite = FlatFile.getInstance();
		for (HashMap<String, Object> ProcessRow : resultListRows) {
			String sQueries = 

(String) ProcessRow.get("Queries");
			String sDBField = (String) ProcessRow.get("DBField");
			String[] 

sDBFields = sDBField.split(",");
			if (sQueries.contains("$AccountNumber$") && sQueries.contains("$PolicyNumber$")) {
...
sReadWrite.Read("PolicyNumber", FlatFileName());
				sQueries = sQueries.replace("$AccountNumber$", strAccountNumber);
...
= sReadWrite.Read("AccountNumber", SCRCommon.FlatFileName());
				sQueries = sQueries.replace("$AccountNumber$", 

strAccountNumber);
				sQueries = sQueries.replace("$SchemaName$", Schema);
			} else if 

(sQueries.contains("$PolicyNumber$")) {
...
strQueueName = sReadWrite.Read("QueueName", FlatFileName());
			...
			sQueries = sQueries.replace("$SchemaName$", Schema);
			} else if (sQueries.contains("$UserFirstName$") 

&& sQueries.contains("$UserLastName$")) {
...
Counter = 0;
			for (int k = 1; k <= odsWait / 10000; k++) {
				resultSet = ConnectDB(sQueries, URL, 
...
sDBAliasName = true;
				...
				Status = sReadWrite.write(PCThreadCache.getInstance().getProperty("TCID"), PCThreadCache.getInstance

().getProperty("methodName"), strFuncValue, queryDBValue, "OUTPUT", sFileName);
							}
...
if (sQueryFound) {
	...
	break;
		...
		Counter++;
					logger.info("ODS did not executed properly for getting DB input hence iterating " 

+ Counter + " for the Query " + sDBFields[0] + "");
					SCRCommon.ODSWait(Integer.valueOf

(HTML.properties.getProperty("WAIT")));
				} else {
					break;
				}
...
().getProperty("testcasename"), PCThreadCache.getInstance().getProperty("methodName"), "ODS should retrieve the value for the query " + 

sDBFields[0] + "", "ODS did not retrieve the value for the query " + sDBFields[0] + "", "FAIL");
			}
		}
...
null)) {
			blnStatus = sReadWrite.write(PCThreadCache.getInstance().getProperty("TCID"), PCThreadCache.getInstance

().getProperty("methodName"), strFuncValue, strLoginUserName, "OUTPUT", FlatFileName());
		}
		return blnStatus;
