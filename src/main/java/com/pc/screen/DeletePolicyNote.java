...
(),PCThreadCache.getInstance().getProperty("TCID"));
	...
	public Boolean SCRDeletePolicyNote() throws Exception {
		...
		/*
		 * if(common.WaitUntilClickable(Common.o.getObject("elePolicyNotePage"),
		 ...
		 * "System should display Policy Notes search Page"
		 * ,"System displayed Policy Notes search Page", "PASS"); status = true; } else
		 * { logger.info("System not displayed Policy Notes search Page");
		 * HTML.fnInsertResult(PCThreadCache.getInstance().getProperty("testcasename"),
		 * PCThreadCache.getInstance().getProperty("methodName"),
		 * "System should display Policy Notes search Page"
		 * ,"System not displayed Policy Notes search Page", "FAIL"); status = false; }
		 */
		return status;
	}
...
"System should display Policy Notes Results", "System displayed Policy notes results - Account Notes Name -" + sstrVal, "PASS");
				sStatus = true;
			} else {
				logger.info("System not displayed Policy Notes Results");
				HTML.fnInsertResult(HTML.properties.getProperty("testcasename"), HTML.properties.getProperty("methodName"), 

"System should display Policy Notes Results", "System not displayed Policy notes results", "FAIL");
				sStatus = false;
			}
			break;
		}
